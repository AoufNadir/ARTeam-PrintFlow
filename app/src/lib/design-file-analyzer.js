...
exports.DesignFileAnalysisError = void 0;
exports.analyzeDesignFile = analyzeDesignFile;
exports.recalculateJpegDimensions = recalculateJpegDimensions;
...
class DesignFileAnalysisError extends Error {
    constructor(message) {
        super(message);
        this.name = 'DesignFileAnalysisError';
    }
}
exports.DesignFileAnalysisError = DesignFileAnalysisError;
...
async function checksumOf(bytes) {
...
function detectFormat(file, bytes) {
    ...
    if (extension === 'ai') {
        if (pdf)
            return { format: 'ai' };
        if (postscript) {
            throw new DesignFileAnalysisError('ملف AI قديم وغير متوافق مع PDF. أعد حفظه من Illustrator مع تفعيل Create PDF Compatible File ثم ارفعه مجدداً.');
        }
        throw new DesignFileAnalysisError('ملف AI غير صالح أو غير متوافق مع PDF.');
    }
    if (pdf)
        return { format: 'pdf', warning: extension !== 'pdf' ? 'تم التعرف على المحتوى كملف PDF رغم اختلاف الامتداد.' : undefined };
    if (svg)
        return { format: 'svg', warning: extension !== 'svg' ? 'تم التعرف على المحتوى كملف SVG رغم اختلاف الامتداد.' : undefined };
    if (jpeg)
        return { format: 'jpg', warning: !['jpg', 'jpeg'].includes(extension) ? 'تم التعرف على المحتوى كصورة JPEG رغم اختلاف الامتداد.' : undefined };
    if (postscript) {
        throw new DesignFileAnalysisError('ملف PostScript/AI القديم غير مدعوم داخل المتصفح. احفظه كـPDF متوافق ثم أعد الرفع.');
    }
    throw new DesignFileAnalysisError('نوع الملف غير معروف. الأنواع المدعومة: PDF وSVG وAI المتوافق مع PDF وJPG.');
}
function compactDataUrl(canvas) {
    return canvas.toDataURL('image/jpeg', 0.76);
}
async function imagePreview(source, maxEdge = 420) {
    ...
    try {
        ...
        if (!context)
            throw new Error('تعذر إنشاء مساحة معاينة.');
...
function hasPdfCutContour(bytes) {
    const searchable = new TextDecoder('latin1').decode(bytes.subarray(0, Math.min(bytes.length, 12000000)));
...
async function analyzePdf(file, bytes, format, inheritedWarning) {
    const [{ PDFDocument, PDFName }, pdfjs] = await Promise.all([
        Promise.resolve().then(() => __importStar(require('pdf-lib'))),
        Promise.resolve().then(() => __importStar(require('pdfjs-dist'))),
    ]);
    let pdfDocument;
    try {
    ...
    if (pages.length === 0)
        throw new DesignFileAnalysisError('الملف لا يحتوي على صفحات.');
    ...
    for (let index = 0; index < pages.length; index += 1) {
        ...
        const warnings = [];
        if (inheritedWarning)
            warnings.push(inheritedWarning);
        if (!hasTrim) {
            warnings.push(hasCrop
                ? 'لا يوجد TrimBox؛ استُخدم CropBox لتحديد القياس.'
                : 'لا يوجد TrimBox أو CropBox؛ استُخدم MediaBox ويُنصح بمراجعة القياس.');
        }
        if (hasTrim && !hasBleed)
            warnings.push('يوجد TrimBox لكن لا يوجد BleedBox؛ راجع قيمة الـBleed قبل الطباعة.');
        if (hasEmbeddedCutContour)
            warnings.push('تم اكتشاف اسم/طبقة يُحتمل أنها مسار قص داخل الملف.');
        let detectedBleedMm;
        if (hasTrim && hasBleed) {
            ...
            if (Math.max(...Object.values(detectedBleedMm)) === 0)
                detectedBleedMm = undefined;
        }
        let previewDataUrl;
...
function sanitizeSvg(documentNode) {
    ...
    for (const style of Array.from(documentNode.querySelectorAll('style'))) {
        if (/@import|url\s*\(\s*['"]?https?:/i.test(style.textContent ?? ''))
            style.remove();
    }
...
function svgCutContour(documentNode) {
...
async function analyzeSvg(file, bytes, inheritedWarning) {
    ...
    if (hasEmbeddedCutContour)
        warnings.push('تم اكتشاف طبقة أو لون يُحتمل أنه مسار قص داخل SVG.');
    let previewDataUrl;
    try {
...
function readU16(bytes, offset, littleEndian = false) {
    return littleEndian
...
function parseExifResolution(bytes, start, end) {
    if (start + 14 > end)
        return {};
    ...
    if (!littleEndian && !bigEndian)
        return {};
    if (readU16(bytes, start + 2, littleEndian) !== 42)
        return {};
...
async function analyzeJpeg(file, bytes, inheritedWarning) {
    ...
    if (pixelWidth <= 0 || pixelHeight <= 0)
        throw new DesignFileAnalysisError('تعذر قراءة أبعاد صورة JPG.');
    ...
    if (inheritedWarning)
        warnings.push(inheritedWarning);
    if (dpiAssumed)
        warnings.push('لا يحتوي JPG على DPI موثوق؛ استُخدم 300 DPI مؤقتاً. راجع القياس قبل الاعتماد.');
    if (!dpiAssumed && dpi < 200)
        warnings.push(`دقة الصورة ${Math.round(dpi)} DPI منخفضة للطباعة بالحجم المكتشف.`);
    else if (!dpiAssumed && dpi < 300)
        warnings.push(`دقة الصورة ${Math.round(dpi)} DPI متوسطة؛ يُفضّل 300 DPI للطباعة الدقيقة.`);
...
async function analyzeDesignFile(file) {
    if (file.size <= 0)
        throw new DesignFileAnalysisError('الملف فارغ.');
    if (file.size > MAX_FILE_BYTES)
        throw new DesignFileAnalysisError('حجم الملف أكبر من 100MB.');
...
function recalculateJpegDimensions(page, dpi) {
    if (!page.pixelWidth || !page.pixelHeight || !Number.isFinite(dpi) || dpi <= 0)
        return page;
    return {
        ...page,
        dpi,
