...
exports.SNAP_THRESHOLD_MM = void 0;
exports.computeSnap = computeSnap;
/** عتبة الالتصاق الافتراضية (مم) — قابلة للتجاوز */
exports.SNAP_THRESHOLD_MM = 2;
const EPS = 1e-6;
/** يختار أقرب مرشح التصاق لمحور واحد (حواف + مراكز القطع المتحركة × الثابتة + المراجع) */
function bestCandidate(moving, statics, refs, threshold, axis) {
    let best = null;
    const consider = (cand) => {
        if (Math.abs(cand.delta) > threshold + EPS)
            return;
        if (!best ||
    ...
    const intervalGap = (a1, a2, b1, b2) => Math.max(0, Math.max(a1, b1) - Math.min(a2, b2));
    for (const m of moving) {
        const mEdges = axis === 'x'
            ? [m.x, m.x + m.w / 2, m.x + m.w]
            : [m.y, m.y + m.h / 2, m.y + m.h];
        const mFrom = axis === 'x' ? m.y : m.x;
        const mTo = axis === 'x' ? m.y + m.h : m.x + m.w;
        for (const s of statics) {
            const sEdges = axis === 'x'
                ? [s.x, s.x + s.w / 2, s.x + s.w]
                : [s.y, s.y + s.h / 2, s.y + s.h];
 ...
 */
function computeSnap(moving, statics, ctx = {}, threshold = exports.SNAP_THRESHOLD_MM) {
    const guides = [];
    let dx = 0;
    let dy = 0;
    let snappedX = false;
    ...
    if (moving.length > 0) {
        ...
        for (const s of statics) {
            ...
            if (s.x + s.w <= bx1 + eps) {
                ...
                if (!best || gap < best.gap)
                    best = { gap, from: s.x + s.w, at: (ov1 + ov2) / 2 };
            }
        }
        ...
        for (const s of statics) {
            ...
            if (s.x >= bx2 - eps) {
                const gap = s.x - bx2;
                if (!best || gap < best.gap)
                    best = { gap, from: s.x, at: (ov1 + ov2) / 2 };
            }
        }
        ...
        for (const s of statics) {
            ...
            if (s.y + s.h <= by1 + eps) {
                ...
                if (!best || gap < best.gap)
                    best = { gap, from: s.y + s.h, at: (ov1 + ov2) / 2 };
            }
        }
        ...
        for (const s of statics) {
            ...
            if (s.y >= by2 - eps) {
                const gap = s.y - by2;
                if (!best || gap < best.gap)
                    best = { gap, from: s.y, at: (ov1 + ov2) / 2 };
            }
        }
